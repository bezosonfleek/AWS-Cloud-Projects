...
- Launch S3 bucket using Terraform: terraform apply
  ...
  Confirm in the management console that an S3 bucket is created. 
- Upload S3 object with Terraform

  Update the main.tf with the code in image.tf which is for uploading an object (image in this case).
  terraform plan, terraform apply

- Use 'terraform destroy' to delete the bucket. Delete the access keys as well, since they are not in use. 
